let scene, camera, renderer, clock;
    ...
    function init() {
      ...
      // 加载初始静态模型
      loadStaticModel('assets/can.glb');

      // Open 动画
      document.getElementById("btn").addEventListener('click', async () => {
        await loadModel('assets/soda_can_opening.glb', false);
        if (actions.length > 0) {
      ...
      document.getElementById("switchModel").addEventListener('click', async () => {
        await loadModel('assets/can-animation-crush.glb', true);
